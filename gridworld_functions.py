...
def generateActions(env):  
    ...
    if (y != 0)          and isEmpty(car, blocks, "up"): actions.append('^')
    if (y != height - 1) and isEmpty(car, blocks, "down"): actions.append('v')
    if (x != width - 1)  and isEmpty(car, blocks, "right"): actions.append('>')
    if (x != 0)          and isEmpty(car, blocks, "left"): actions.append('<')
...
def isGoal(env):
    for row in env['grid']:
        if 2 in row:    #returns False if it finds (2 -> goal) in the grid and True otherwise.
            return False
    return True
