...
def shuffle(size, car, goal):
	...
	env['grid'] = [[random.choice(numbers) for i in range(size[0])] for j in range(size[1])]
...
def computerSolve(env, strategy, h = None, flag = False):
	S = ai_search.solve(strategy, env, generateActions, applyAction, isGoal, computeCost, h)
	print(strategy)
	if S == "failure": 
		print(S)
		return
